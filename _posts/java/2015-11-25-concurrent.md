...
1.  无状态对象一定是线程安全的
2.  不可变对象一定是线程安全的
3.  线程安全的程序不一定完全由线程安全类构成，线程安全类构成的程序不一定就是线程安全的，线程安全类中也可以包含非线程安全的类
...
2.  静态方法上加synchronized　是对Class对象作为锁
3.  内置锁是互斥锁
...
*   对变量的写入操作布衣赖变量的当前值，或者你能确保只有单个线程更新变量的值
...
怎么理解同步、异步、阻塞、非阻塞？　　
　
1.  **同步和异步关注的是调用方和被调用方之间的消息通信机制；牵涉调用方和被调用方；　　　　　　　　　
对于调用方，前者是主动去拉结果，后者是被动的接受被调用方推来的结果（可以是消息或者回调）**
2.  **阻塞和非阻塞关注的是调用方在等待调用结果（消息，返回值）时的状态；只牵涉调用方；　　　　　　　　
前者是线程挂起，一直在等，期间不能干其他的事，直到有结果（可以是自己拉结果，也可以是等着推结果），后者是时不时去check(同前，可以是自己拉的结果，也可以是被推的结果)，期间可以干其他的事**
